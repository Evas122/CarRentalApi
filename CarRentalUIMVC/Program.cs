...
builder.Services.AddTransient<IRentalDetailRepository, RentalDetailRepository>();
builder.Services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(GetCarModelsQuery).Assembly));
builder.Services.AddDbContext<CarRentalContext>(options => options.UseSqlServer(builder.Configuration.GetConnectionString("CarRentalContext")));
